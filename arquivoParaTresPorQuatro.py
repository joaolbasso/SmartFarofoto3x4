...
def manual_crop_interface(img):
    root = tk.Tk()
    root.withdraw()

    img_pil_orig = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))

    display_width, display_height = img_pil_orig.size
    max_display_dim = 900
    if display_width > max_display_dim or display_height > max_display_dim:
        scale_factor = max_display_dim / max(display_width, display_height)
        img_pil_display = img_pil_orig.resize(
            (int(display_width * scale_factor), int(display_height * scale_factor)), Image.LANCZOS)
    else:
        img_pil_display = img_pil_orig

    img_tk = ImageTk.PhotoImage(img_pil_display)

    top = tk.Toplevel(root)
    top.title("Recorte Manual - Ajuste Necessário")

    canvas = tk.Canvas(top, width=img_tk.width(),
                       height=img_tk.height(), cursor="cross")
    ...
    def confirm_crop():
        ...
        if rect_id:
            ...
            scale_factor = img_pil_orig.width / img_pil_display.width
            crop_coords = [
                int(min(x1, x2) * scale_factor),
                int(min(y1, y2) * scale_factor),
                int(max(x1, x2) * scale_factor),
                int(max(y1, y2) * scale_factor)
            ]
        top.destroy()
        root.quit()
    ...
    tk.Button(top, text="Confirmar Recorte", command=confirm_crop).pack(
        side=tk.LEFT, padx=5, pady=5)
    tk.Button(top, text="Cancelar", command=cancel_crop).pack(
    ...
    final_photo = cv2.resize(
        cropped_img_raw, (LARGURA_PX, ALTURA_PX), interpolation=cv2.INTER_AREA)
    return final_photo
